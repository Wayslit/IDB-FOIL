...
def get_result_list(target,result_list,total_list,variable1):
    new_result_list=[]
    number=[]
    character=[]
...
def threshold(target,clause,total_list,variable1):
...
def foil(target_list,target,total_list,variable1,rule_base,decay_dir,foil_gain):

    result_list=[]     
    object_list=["ACDR","HCDR","VCDR"]
                ...
                for clause_number,clause_gain in enumerate(foil_gain_list):
                    ...
                    if clause_gain==max(foil_gain_list):
                        ...
                        if a[0]=="overlap":
                            ...
                            if a[1] in parameter_list and threshold(target,possible_clause[clause_number],total_list,variable1)!=False:
...
def FOIL(input_list, rule_save_path, rule_base, decay_dir, foil_gain):
    ...
    for target in target_list:
        result_list=foil(target_list,target,total_list,global_variable1,rule_base,decay_dir,foil_gain=foil_gain)
        if result_list==None:
            dict_math[target]=[['none']]
        else:
            math_format=get_result_list(target,result_list,total_list,global_variable1)
            dict_math[target]=math_format
            dict_math = set_rules(dict_math)
